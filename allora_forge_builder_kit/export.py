...
Generic across pair/timeframe: PAIR, TIMEFRAME and PREDICTION_KIND are read from
the environment at runtime (the hosting platform sets them per deployment), so one
package can be deployed against many pairs/timeframes/topics. Model-intrinsic config (features, target,
...
def _prediction_kind() -> str:
    # The model always trains a log-return target; whether a deployment publishes
    # the raw log-return or an absolute price is a per-deployment topic concern,
    # so it rides the same env channel as PAIR/TIMEFRAME (never baked into the
    # package). Defaults to the model's native output; set PREDICTION_KIND=price
    # to convert to an absolute price for price topics.
    kind = os.environ.get("PREDICTION_KIND", "log_return").strip().lower()
    if kind not in ("log_return", "price"):
        raise RuntimeError(f"PREDICTION_KIND must be 'log_return' or 'price', got {kind!r}")
    return kind
    ...
    async def get_inference(self, pair: str, **kwargs):
        ...
        if _prediction_kind() == "price":
            ...
            if not (np.isfinite(current_price) and current_price > 0):
                raise ValueError(
                    "PREDICTION_KIND=price requires a positive current_price, "
                    f"got {current_price!r}"
                )
