...
Generic across pair/timeframe: PAIR and TIMEFRAME are read from the environment at
runtime (the hosting platform sets them per deployment), so one package can be
deployed against many pairs/timeframes/topics. Whether inference publishes the
model's native log-return or an absolute price is driven by ``config.submit_returns``,
which the SDK auto-derives from the topic's on-chain loss_method (SUBMIT_RETURNS),
not a value the package bakes in. Model-intrinsic config (features, target,
...
def _env_submit_returns() -> bool:
    # The SDK auto-derives SUBMIT_RETURNS from the topic's loss_method: on "auto"
    # (or unset) it queries the chain and sets config.submit_returns after this
    # config is built; on explicit "true"/"false" it skips the query and relies on
    # the model config having read the env. So parse it here, defaulting to True
    # (publish the model's native log-return) when unresolved.
    return os.environ.get("SUBMIT_RETURNS", "").strip().lower() != "false"


@dataclasses.dataclass
class ForgeModelConfig(ModelConfig):
    # submit_returns must exist on the config for the SDK's autoresolve to populate
    # it (it gates on hasattr(model.config, "submit_returns")) and to honor an
    # explicit SUBMIT_RETURNS=true|false. True => publish the raw log-return;
    # False => convert to an absolute price. Not baked into config.json — it is a
    # per-topic signal, resolved at deploy time like PAIR/TIMEFRAME.
    submit_returns: bool = dataclasses.field(default_factory=_env_submit_returns)
...
class ForgeModel(BaseModel):
    ...
    @classmethod
    def default_config(cls, **kwargs) -> ModelConfig:
        # Return ForgeModelConfig (not the base ModelConfig) so config.submit_returns
        # exists — the SDK gates SUBMIT_RETURNS resolution on its presence.
        valid = {f.name for f in dataclasses.fields(ForgeModelConfig)}
        return ForgeModelConfig(**{k: v for k, v in kwargs.items() if k in valid})
    ...
    async def get_inference(self, pair: str, **kwargs):
        ...
        # submit_returns is the SDK-derived per-topic signal (True => log-return
        # topic). Only convert to an absolute price when it is explicitly False, so
        # an unresolved signal never silently emits a price for a returns topic.
        if getattr(self.config, "submit_returns", True) is False:
            # Price topics want an absolute price; converting a log-return needs the
            # current price, so a missing one is a hard error — silently publishing
            # the raw ~0.00x log-return into a price topic would be far worse.
            if not (np.isfinite(current_price) and current_price > 0):
                raise ValueError(
                    "submit_returns=False (price topic) requires a positive "
                    f"current_price, got {current_price!r}"
                )
