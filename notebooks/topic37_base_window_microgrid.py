#!/usr/bin/env python3
"""Topic 37 micro-grid around best base geometry (5m).

Focused sweep for fast iteration:
- interval fixed to 5m
- bars around prior winner
- denser Ridge alpha grid around 0.3
"""
...
def main():
    ...
    bars_grid = [36, 48, 60, 72]
    alpha_grid = [0.05, 0.1, 0.2, 0.3, 0.5, 0.8, 1.2]
    ...
    for n_bars in bars_grid:
        ...
        b = out["best"]
        print(f"best alpha={b['alpha']} pass={b['num_passed']}/8 score={b['score']:.1%} DA={b['metrics']['directional_accuracy']:.4f}")
        results.append(out)
    ...
    overall = sorted([
        ...
        for r in results
    ], key=lambda r: (r["num_passed"], r["score"], r["metrics"]["czar_improvement"]), reverse=True)
    ...
    out_path = Path(__file__).parent / "artifacts" / "topic37_base_window_microgrid_report.json"
