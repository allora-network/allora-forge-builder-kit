#!/usr/bin/env python3
"""Topic 37 tiny-bars microgrid (5m, <10 input bars).

Goal:
- Start from very small history windows (2..9 bars)
- Use simple linear models first (Ridge)
- Identify whether signal quality improves before expanding complexity
"""
...
def run_combo(api_key: str, days: int, splits: int, n_bars: int, alphas: list[float], skip_backfill: bool):
    wf = AlloraMLWorkflow(
        tickers=["solusd"],
    ...
    if len(df) < 200:
        raise ValueError(f"Not enough rows for bars={n_bars}: {len(df)}")
...
def main():
    ...
    bars_grid = [2, 3, 4, 5, 6, 7, 8, 9]
    alpha_grid = [0.03, 0.1, 0.3, 1.0, 3.0]
    ...
    for n_bars in bars_grid:
        print(f"\n=== 5m bars={n_bars} ===")
        out = run_combo(api_key, args.days, args.splits, n_bars, alpha_grid, args.skip_backfill)
        b = out["best"]
        print(
            f"best alpha={b['alpha']} pass={b['num_passed']}/8 "
            f"score={b['score']:.1%} DA={b['metrics']['directional_accuracy']:.4f} "
            f"r={b['metrics']['pearson_r']:.4f}"
        )
        results.append(out)
    ...
    out_path = Path(__file__).parent / "artifacts" / "topic37_tinybars_grid_report.json"
