...
NUMBER_OF_INPUT_BARS = 289  # 289 prices span 288 × 5 min = 24 hours
TARGET_BARS = 96           # 96 × 5 min = 8-hour prediction horizon
...
def engineer_returns(row):
    ...
    returns['log_return_24h'] = np.log(closes[-1] + 1e-8) - np.log(closes[-289] + 1e-8) if NUMBER_OF_INPUT_BARS >= 289 else 0
...
pkl_path = os.path.join(os.path.dirname(__file__), "predict_71.pkl")
...
print("="*80)
print("\nDeploy (from notebooks/): TOPIC_ID=71 PREDICT_PKL=testnet/topic_71_near_8h_logreturn/predict_71.pkl python deploy_worker.py")
