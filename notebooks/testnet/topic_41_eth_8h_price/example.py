...
for rank_idx, (cfg_num, params, model, selected, row) in enumerate(trained_models):
    ...
    def _make_predict(m, sel, _base_feature_cols=base_feature_cols[:],
        ...
        def predict(nonce=None):
            ...
            predicted_log_return = booster.predict(live_features[_sel].values.reshape(1, -1))[0]
            predicted_price = current_price * np.exp(predicted_log_return)
            return float(predicted_price)
        return predict
    
    predict_fn = _make_predict(model, selected)
    pkl_name = os.path.join(os.path.dirname(__file__), f"predict_41_rank{rank_idx+1}.pkl")
...
with open(os.path.join(os.path.dirname(__file__), "predict_41.pkl"), "wb") as f:
